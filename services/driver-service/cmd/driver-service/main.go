...
func main() {
	cfg := config.Load("driver-service", 8083)
	logger := logx.New(cfg.Name)
	r := chi.NewRouter()
	r.Get("/healthz", func(w http.ResponseWriter, _ *http.Request) {
		httpx.JSON(w, http.StatusOK, map[string]any{"service": cfg.Name, "status": "ok"})
	})
	r.Post("/v1/drivers/location", func(w http.ResponseWriter, _ *http.Request) {
		httpx.JSON(w, http.StatusAccepted, map[string]any{"state": "location_ingested"})
	})
