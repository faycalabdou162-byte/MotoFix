...
func main() {
	...
	consumer := kafka.NewConsumer(cfg.KafkaBrokers, "notification-service", "mobility-events")
	...
	r.Get("/v1/notifications/ping", func(w http.ResponseWriter, r *http.Request) {
...
func loop(ctx context.Context, consumer *kafka.Consumer) {
	for {
		...
		case events.EventDispatchOffered, events.EventTripAccepted, events.EventPaymentAuthorized:
		default:
		}
